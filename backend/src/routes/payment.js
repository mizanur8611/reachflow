...
async function creditWallet(userId, amount, method, description = '') {
  return await prisma.$transaction(async (tx) => {
    ...
    await tx.transaction.create({
      data: { walletId: wallet.id, type: 'DEPOSIT', amount, status: 'COMPLETED', method, description }
    })
    return wallet
  })
}

async function deductWallet(userId, amount, method, description = '', type = 'CAMPAIGN_PAYMENT') {
  return await prisma.$transaction(async (tx) => {
    ...
    await tx.transaction.create({
      data: { walletId: wallet.id, type, amount, status: 'COMPLETED', method, description }
    })
    return updated
  })
}
...
router.post('/stripe/webhook', express.raw({ type: 'application/json' }), async (req, res) => {
  ...
  if (event.type === 'payment_intent.succeeded') {
    ...
    await creditWallet(intent.metadata.userId, amount, 'STRIPE', `Stripe deposit: ${intent.id}`)
  }
...
router.get('/bkash/callback', async (req, res) => {
  try {
    ...
    if (execute.data.transactionStatus === 'Completed') {
      ...
      const amountUSD = amountBDT / 110
      await creditWallet(execute.data.payerReference, amountUSD, 'BKASH', `bKash: ${execute.data.trxID} (BDT ${amountBDT})`)
    }
...
router.post('/nagad/create-payment', requireAuth, async (req, res) => {
  try {
    ...
    const plaintext = JSON.stringify({
      ...
      orderId, currencyCode: '050',
      amount: amount.toString(), challenge
    })
    ...
    await prisma.transaction.create({
      data: {
        walletId: wallet.id, type: 'DEPOSIT',
        amount: parseFloat(amount) / 110,
        status: 'PENDING', method: 'NAGAD',
        reference: orderId,
...
router.get('/nagad/callback', async (req, res) => {
  try {
    ...
    if (pendingTx) {
      const amountUSD = parseFloat(pendingTx.metadata.amountBDT) / 110
...
router.post('/withdraw', requireAuth, async (req, res) => {
  try {
    ...
    await prisma.$transaction([
      prisma.withdrawal.create({ data: { promoterId: promoter.id, amount, method, accountInfo, status: 'PENDING' } }),
      prisma.wallet.update({
        where: { userId: req.user.id },
        data: { balance: { decrement: amount }, pending: { increment: amount } }
      }),
      prisma.transaction.create({
        data: { walletId: wallet.id, type: 'WITHDRAWAL', amount, status: 'PENDING', method, description: `Withdrawal via ${method}` }
      })
    ])
...
router.post('/campaign/pay', requireAuth, async (req, res) => {
  try {
    ...
    await deductWallet(req.user.id, amount, 'WALLET', `Campaign payment: ${campaign.title}`, 'CAMPAIGN_PAYMENT')
    await prisma.campaign.update({ where: { id: campaignId }, data: { spentBudget: { increment: amount } } })
